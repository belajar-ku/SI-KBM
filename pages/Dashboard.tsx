...
interface MonthlyStats {
    totalJp: number;
    targetJp: number; // Added Target JP
...
const Dashboard: React.FC = () => {
  ...
  const fetchDashboardData = async () => {
    try {
        ...
        // --- 1. Fetch Journals Stats (Monthly Completed) ---
        ...
        // --- 1.5 Calculate TARGET JP (Based on Schedule & Calendar) ---
        // A. Fetch Schedules
        ...
        let targetJp = 0;
        if (mySchedules) {
            // B. Count occurrences of each day in the current month
            ...
            for (let d = 1; d <= lastDayOfMonth.getDate(); d++) {
                ...
                const dbDay = jsDay === 0 ? 7 : jsDay; // Convert Sun(0)->7
                dayCounts[dbDay]++;
            }

            // C. Sum up JP from schedules * day occurrences
  ...
  if (percentage > 85) {
      ...
      performanceColor = "text-emerald-200";
  } else if (percentage >= 70) {
      performanceStatus = "Sesuai Ekspektasi";
      performanceColor = "text-blue-200";
  }
